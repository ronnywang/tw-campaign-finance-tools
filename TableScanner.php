...
class TableScanner
{
    ...
    public function getCellsFromFile($png_file, $debug = false)
    {
        ...
        $rgb = imagecolorat($gd, floor($width / 2), $height - 1);
        $colors = imagecolorsforindex($gd, $rgb);
        if ($colors['red'] == 0 and $colors['green'] == 0 and $colors['blue'] == 0) {
        ...
        for ($check_y = $top_y; $check_y < $height; $check_y ++ ) {
            ...
            for ($i = 1; $i < $i_limit; $i ++) {
                ...
                for ($x_pos = 1; $x_pos < $width; $x_pos ++) {
                    ...
                    if ($y < 1) {
                        error_log(sprintf("y<1 ?!  y=%f, r=%f, x=%f, theta=%f", $y, $r, $x, $theta));
                    }
            ...
            error_log("找到一條水平線: r={$max_r}, theta={$max_theta}, width={$width}, height={$height}");
